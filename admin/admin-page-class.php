...
if ( ! class_exists( 'myg_Admin_Page_Class') ) :
 ...
 */
  ...
  class myg_Admin_Page_Class {
    ...
    public function plupload_head_js(){
      if ($this->has_field('plupload')){
         $plupload_init = array(
            ...
            'filters'             => array(array('title' => esc_html__('Allowed Files','apc'), 'extensions' => '*')),
            'multipart'           => true,
    ...
    public function edit_insert_to_post_text( $safe_text, $text ) {
      if( is_admin() && 'Insert into Post' == $safe_text){
        if (isset($_REQUEST['apc']) && 'insert_file' == $_REQUEST['apc'] )
          return str_replace(esc_html__('Insert into Post'), esc_html__('Use this File','apc'), $safe_text);
        else
          return str_replace(esc_html__('Insert into Post'), esc_html__('Use this Image','apc'), $safe_text);
      }
      return $safe_text;
    }
    ...
    public function DisplayPage() {
      ...
      echo '<form method="post" name="'.apply_filters('apc_form_name', 'admin_page_class',$this).'" class="'.apply_filters('apc_form_class', 'admin_page_class',$this).'" id="'.apply_filters('apc_form_id', 'admin_page_class',$this).'" action="" enctype="multipart/form-data">
        ...
        <div class="fone_header_logo" style="float:left">'."\n";
        echo apply_filters('admin_page_class_before_title','');
		echo '<a href="http://www.mircode.com" title="Mircode Solution" target="_blank"><img src="'.$this->SelfPath . '/images/mircode-logo-40.png'.'"/></a>';
        ...
        <div style="float:right;margin:32px 0 0 0">'."\n".'
          <input type="submit" style="margin-left: 25px;" value="'.esc_attr(esc_html__('Save Changes','apc')).'" name="Submit" class="'.apply_filters('admin_page_class_submit_class', 'btn-info').' btn"><br><br>
        </div>'."\n".'
      ...
      if ($this->saved_flag){
        ...
        if (is_array($this->errors) && count($this->errors) > 0 ){
          ...
          echo '<div class="alert alert-success"><button data-dismiss="alert" class="close" type="button">×</button><strong>'.esc_html__('Settings saved.','apc').'</strong></div>'."\n";
        }
        echo '</div>'."\n";
      }
    ...
    echo '</div>'."\n".'<div style="clear:both"></div>'."\n".'<div class="footer_wrap">'."\n".'
        <div style="float:right;margin:32px 0 0 0">'."\n".'
          <input type="submit" style="margin-left: 25px;" name="Submit" class="'.apply_filters('admin_page_class_submit_class', 'btn-info').' btn" value="'.esc_attr(esc_html__('Save Changes','apc')).'" />'."\n".'
          <br><br>
        </div>'."\n".'
        <br style="clear:both"><br>
      </div>'."\n";
  ...
  public function load_scripts_styles() {
    ...
    wp_enqueue_style('FancyCheckbox',$plugin_path. '/js/FancyCheckbox/FancyCheckbox.css');
    
    // Enqueue admin page Scripts
    wp_enqueue_script( 'Admin_Page_Class', $plugin_path . '/js/Admin_Page_Class.js', array( 'jquery' ), null, true );
    wp_enqueue_script('FancyCheckbox',$plugin_path. '/js/FancyCheckbox/FancyCheckbox.js',array('jquery'),null,true);
  ...
  public function check_field_code() {
    ...
    if ( $this->has_field( 'code' ) && $this->is_edit_page() ) {
      ...
      wp_enqueue_style( 'codemirror', $plugin_path .'/js/codemirror/codemirror.css',array(),null);
      wp_enqueue_style( 'solarizedDark', $plugin_path .'/js/codemirror/solarizedDark.css',array(),null);
      wp_enqueue_style( 'solarizedLight', $plugin_path .'/js/codemirror/solarizedLight.css',array(),null);
      wp_enqueue_script('codemirror',$plugin_path .'/js/codemirror/codemirror.js',array('jquery'),false,true);
      wp_enqueue_script('xml',$plugin_path .'/js/codemirror/xml.js',array('jquery'),false,true);
      wp_enqueue_script('javascript',$plugin_path .'/js/codemirror/javascript.js',array('jquery'),false,true);
      wp_enqueue_script('css',$plugin_path .'/js/codemirror/css.js',array('jquery'),false,true);
      wp_enqueue_script('clike',$plugin_path .'/js/codemirror/clike.js',array('jquery'),false,true);
      wp_enqueue_script('php',$plugin_path .'/js/codemirror/php.js',array('jquery'),false,true);
    }
  }
  ...
  public function insert_images() {
    ...
    $id = sanitize_text_field($_POST['field_id']);
    ...
    foreach( $_POST['attachments'] as $attachment_id => $attachment ) {
      ...
      $li   .= "<a title='" . esc_html__( 'Delete this image','apc' ) . "' class='at-delete-file' href='#' rel='{$nonce}|{$post_id}|{$id}|{$attachment_id}'><img src='" . $this->SelfPath. "/images/delete-16.png' alt='" . esc_html__( 'Delete' ,'apc') . "' /></a>";
  ...
  */
  public function wp_ajax_delete_image() {
    ...
    if ( $ok ){
      ...
      echo json_encode(array('message' => esc_html__( 'Cannot delete file. Something\'s wrong.','apc')));
      die();
    }
  }
  ...
  public function reorder_images() {
    ...
    foreach ( $items as $item ) {
      wp_update_post( array( 'ID' => $item, 'post_parent' => sanitize_text_field($post_id), 'menu_order' => sanitize_text_field($order) ) );
      $order++;
    }
  ...
  public function show_field_repeater( $field, $meta ) {
      ...
      if (count($meta) > 0 && is_array($meta) ){
        ...
        <span class="at-re-toggle"><img src="';
           if ($this->_Local_images){
             ...
             echo $plugin_path.'/images/ka0E2.png';
           }
        ...
        if ($this->_Local_images){
          ...
          echo $plugin_path.'/images/g8Duj.png';
        }
        echo '" alt="'.esc_html__('Remove','apc').'" title="'.esc_html__('Remove','apc').'" id="remove-'.$field['id'].'"></div>';
        $c = $c + 1;
    ...
    if ($this->_Local_images){
      ...
      echo $plugin_path.'/images/w5Tuc.png';
    }
    echo '" alt="'.esc_html__('Add','apc').'" title="'.esc_html__('Add','apc').'" id="add-'.$jsid.'"><br/></div>';
    ...
    echo '" alt="'.esc_html__('Remove','apc').'" title="'.esc_html__('Remove','apc').'" id="remove-'.$jsid.'"></div>'."\n";
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
  ...
  public function show_field_plupload( $field, $meta) {  
    ...
    $html = '
        ...
        <div class="plupload-upload-uic hide-if-no-js '.$m1.'" id="'.$id.'plupload-upload-ui">
          <input id="'.$id.'plupload-browse-button" type="button" value="'.esc_html__('Select Files','apc').'" class="button" />
  ...
  public function show_field_file( $field, $meta ) {
    ...
    $this->show_field_begin( $field, $meta );
      ...
      if ( ! empty( $meta ) ) {
        $nonce = wp_create_nonce( 'at_ajax_delete' );
        echo '<div style="margin-bottom: 10px"><strong>' . esc_html__('Uploaded files','apc') . '</strong></div>';
        echo '<ol class="at-upload">';
        foreach ( $meta as $att ) {
          // if (wp_attachment_is_image($att)) continue; // what's image uploader for?
          echo "<li>" . wp_get_attachment_link( $att, '' , false, false, ' ' ) . " (<a class='at-delete-file' href='#' rel='{$nonce}|{$post->ID}|{$field['id']}|{$att}'>" . esc_html__( 'Delete' ,'apc') . "</a>)</li>";
        }
        echo '</ol>';
      }
      ...
      echo "<div class='at-file-upload-label'>";
        echo "<strong>" . esc_html__( 'Upload new files' ,'apc') . "</strong>";
      echo "</div>";
      echo "<div class='new-files'>";
        echo "<div class='file-input'>";
          echo "<input type='file' name='{$field['id']}[]' />";
        echo "</div><!-- End .file-input -->";
        echo "<a class='at-add-file button' href='#'>" . esc_html__( 'Add more files','apc' ) . "</a>";
  ...
  public function show_field_media_manager($field,$meta){
    ...
    if (is_array($meta) && isset($meta['src']) && $meta['src'] != ''){
      ...
      $html .= "<input class='at-delete_image_button button' type='button' rel='".$field['id']."' value='".esc_html__('Delete Image','apc')."' />"."\n";
    }else{
      ...
      $html .= "<input class='at-mm-upload_image_button button' type='button' rel='".$field['id']."' value='".esc_html__('Upload Image','apc')."' />"."\n";
    }
    echo $html;
    $this->show_field_end( $field, $meta );
  }
  ...
  public function show_field_image( $field, $meta ) {
    ...
    if (is_array($meta) && isset($meta['src']) && $meta['src'] != ''){
      ...
      $html .= "<input type='hidden' name='".$field['id']."[id]' id='".$field['id']."[id]' value='".$meta['id']."' />";
      $html .= "<input type='hidden' name='".$field['id']."[src]' id='".$field['id']."[src]' value='".$meta['src']."' />";
      $html .= "<input class='at-delete_image_button button' type='button' data-u='".$upload_type."' rel='".$field['id']."' value='".esc_html__('Delete Image','apc')."' />";
    }else{
      $html .= "<span class='mupload_img_holder'  data-wi='".$width."' data-he='".$height."'></span>";
      $html .= "<input type='hidden' name='".$field['id']."[id]' id='".$field['id']."[id]' value='' />";
      $html .= "<input type='hidden' name='".$field['id']."[src]' id='".$field['id']."[src]' value='' />";
      $html .= "<input class='at-upload_image_button button' type='button' data-u='".$upload_type."' rel='".$field['id']."' value='".esc_html__('Upload Image','apc')."' />";
    }
    echo $html;
    $this->show_field_end( $field, $meta );
  }
  ...
  public function show_field_typo( $field, $meta ) {
    ...
    if( wp_style_is( 'wp-color-picker', 'registered' ) ) { //iris color picker since 3.5
      ...
      $html .= "<input type='button' class='at-color-select button' rel='".$field['id']."[color]' value='" . esc_html__( 'Select a color' ,'apc') . "'/>"."\n";
  ...
  public function show_field_color( $field, $meta ) {
    ...
    if( wp_style_is( 'wp-color-picker', 'registered' ) ) { //iris color picker since 3.5
      ...
      echo "<input type='button' class='at-color-select button' rel='{$field['id']}' value='" . esc_html__( 'Select a color' ,'apc') . "'/>";
  ...
  public function show_import_export(){
    ...
    <div class="apc_ie_panel field">
      <div style="padding 10px;" class="apc_export"><h3>'.esc_html__('Export','apc').'</h3>
        <p>'. esc_html__('To export saved settings click the Export button bellow and you will get the export Code in the box bellow, which you can later use to import.','apc').'</p>
        <div class="export_code">
          <label for="export_code">'. esc_html__('Export Code','apc').'</label><br/>
          <textarea id="export_code"></textarea>        
          <input class="button" type="button" value="'. esc_html__('Get Export','apc').'" id="apc_export_b" />'.$this->create_export_download_link().'
          <div class="export_status" style="display: none;"><img src="'.$this->SelfPath.'/images/l4pWs.gif" alt="loading..."/></div>
          <div class="export_results alert" style="display: none;"></div>
        </div>
      </div>
      <div style="padding 10px;" class="apc_import"><h3>'.esc_html__('Import','apc').'</h3>
        <p>'. esc_html__('To Import saved settings paste the Export output in to the Import Code box bellow and click Import.','apc').'</p>
        <div class="import_code">
          <label for="import_code">'. esc_html__('Import Code','apc').'</label><br/>
          <textarea id="import_code"></textarea>
                  <input class="button" type="button"  value="'. esc_html__('Import','apc').'" id="apc_import_b" />
          <div class="import_status" style="display: none;"><img src="'.$this->SelfPath.'/images/l4pWs.gif" alt="loading..."/></div>
  ...
  public function export(){
    ...
    if (!isset($_GET['group'])){
      $re['err'] = esc_html__('error in ajax request! (1)','apc');
    ...
    if ($options !== false)
      ...
      $re['err'] = esc_html__('error in ajax request! (2)','apc');
  ...
  public function import(){
    ...
    if (!isset($_POST['imp'])){
      $re['err'] = esc_html__('error in ajax request! (3)','apc');
    ...
    $import_code = sanitize_text_field(unserialize($import_code));
    if (is_array($import_code)){
      update_option($this->option_group,$import_code);
      $re['success']= esc_html__('Setting imported, make sure you ','apc'). '<input class="button-primary" type="button"  value="'. esc_html__('Refresh this page','apc').'" id="apc_refresh_page_b" />';
    }else{
      $re['err'] = esc_html__('Could not import settings! (4)','apc');
    }
    //update_nonce
      $re['nonce'] = wp_create_nonce("apc_import");
    echo json_encode($re);
  ...
  public function download_file($content = null, $file_name = null){
    ...
    if ( !current_user_can('edit_posts') )
        wp_die('<p>'.esc_html__('You do not have sufficient permissions to edit templates for this site.','apc').'</p>');
    
    if ($content === null || $file_name === null){
        wp_die('<p>'.esc_html__('Error Downloading file.','apc').'</p>');     
    }
  ...
  public function create_export_download_link($echo = false){
    ...
    if ($echo === true)
        echo '<a href="'.$export_url.'" target="_blank">'.esc_html__('Download Export','apc').'</a>';
    elseif ($echo == 'url')
        return $export_url;
    return '<a class="button" href="'.$export_url.'" target="_blank">'.esc_html__('Download Export','apc').'</a>';
  }
    ...
    foreach ($field['validate'] as $type => $args) {
      if (method_exists($this,'is_' . $type)){  
        if (call_user_func ( array( $this, 'is_' . $type ), $meta ,$args['param']) === false){
          ...
          $this->errors[$field['id']]['m'][] = (isset($args['message'])? $args['message'] : esc_html__('Not Valid ','apc') . $type);
          $ret = false;
        }
      }
    }
  ...
  public function displayErrors(){
    if ($this->errors_flag){
      ...
      echo '<h4>'.esc_html__('Errors in saving changes', 'apc').'</h4>';
  ...
  public function getFieldErrors($field_id){
    ...
    return esc_html__('Unkown Error','apc');
  }
